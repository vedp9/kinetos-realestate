...
// load embedding model once — reused across requests
let embedder = null
...
export async function POST(req) {
  try {
    ...
    if (!message || !agentSlug) {
      return Response.json({ error: 'Message and agent slug required' }, { status: 400 })
    }

    // 1. get agent from slug
    ...
    // 3. build a simple context string of all properties
    ...
    // 4. ask Groq AI with the property context
...
If no matching property found, say so politely.
